...
library(tidyverse)
library(agricolae)   
setwd("C:/Users/cc743/Desktop/EDA/Environmental_Data_Analytics_2022/Data/Processed")
PeterPaul.chem.nutrients <- read.csv("NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv", stringsAsFactors = TRUE)
# Set date to date format
PeterPaul.chem.nutrients$sampledate <- as.Date(PeterPaul.chem.nutrients$sampledate, format = "%Y-%m-%d")

EPAair <- read.csv("EPAair_O3_PM25_NC1819_Processed.csv", stringsAsFactors = TRUE)
# Set date to date format
EPAair$Date <- as.Date(EPAair$Date, format = "%Y-%m-%d")


Litter <- read.csv("NEON_NIWO_Litter_mass_trap_Processed.csv", stringsAsFactors = TRUE)
# Set date to date format
...
> Answer: The mean AQI for Ozone is below 50.
...
ggplot(data  = EPAair, aes(x = Ozone, fill = factor(Year))) + geom_density(alpha = 0.5) + scale_fill_manual(values = c('blue', 'yellow'))

ggplot(data = EPAair, aes(y = Ozone, x = factor(Year))) + geom_boxplot() + stat_summary(fun = max, geom = "text", label = c("a","b"), vjust = -1)

ggplot(data = EPAair, aes(y = Ozone, x = factor(Year))) + geom_violin(draw_quantiles = 0.5) + stat_summary(fun = max, geom = "text", label = c("a","b"), vjust = -1)
...
O3_P25 <- lm(data = EPAair, Ozone ~ PM2.5)
sumr<-summary(O3_P25)
sumr$r.squared
sumr$coefficients

#Exercise 4: Build a scatterplot. Add a line and standard error for the linear regression. 
ggplot(data = EPAair, aes(x = Ozone, y = PM2.5)) + geom_point() + geom_smooth(method ='lm', se = F)
...
lm0 <- lm(data = EPAair, Ozone ~ SITE_LATITUDE:PM2.5 - SITE_LONGITUDE - Year)
summary(lm0)

cor <- cor(EPAair %>% select(SITE_LATITUDE:PM2.5) %>% drop_na())
library(corrplot)
...
lm1 <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + SITE_LATITUDE + SITE_LONGITUDE)
lm_fin <- step(lm1)
summary(lm_fin)
...
lm2 <- lm(data = EPAair, PM2.5 ~ Ozone + Month + SITE_LATITUDE + SITE_LONGITUDE)
summary(lm2)
