...
huc8_sf$HUC_8 <- as.factor(huc8_sf$HUC_8)
...
UpperNeuse <- st_read("./Data/Spatial/NCHUC8.shp") %>% filter(SUBBASIN == "Upper Neuse")


#Create a map
ggplot() + 
  geom_sf(data=huc8_sf, fill = "orange") + 
  geom_sf(data=UpperNeuse, fill = "purple")
...
wind <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', 
                 colClasses = c("HUC_12" = "factor")) %>% 
  group_by(HUC_12) %>%
  summarise(energy = sum(kWhkm2day)) %>%
  mutate(huc_8 = substr(HUC_12, 1, 8))


#Join to HUC_8 features
left_join(huc8_sf, wind, by = c("HUC_8" = "huc_8"))
