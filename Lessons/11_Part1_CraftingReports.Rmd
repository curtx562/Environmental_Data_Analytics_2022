...
library("pacman")
p_load(tidyverse, lubridate, knitr)
...
NTL_LTER_Lake_Nutrients_Raw <- read_csv("../Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
View(NTL_LTER_Lake_Nutrients_Raw)

NTL_LTER_Lake_Nutrients_Raw$sampledate <- mdy(NTL_LTER_Lake_Nutrients_Raw$sampledate)
...
data_clean <- NTL_LTER_Lake_Nutrients_Raw %>% 
...
```{r Visualize Data Table, echo= FALSE}

kable(data_summary_N, caption = "Summary Statistics for Nitrogen Concentrations")

kable(data_summary_P, caption = "Summary Statistics for Phosphorous Concentrations")
...
```{r Visualize Data Plots N, echo = FALSE, fig.align = "Left", fig.cap = "N (ug) over Time"}

ggplot(data = data_clean, aes(x = sampledate, y = tn_ug, colour = lakename)) + geom_line(linetype = 5, size  = 0.8) +
  labs(x = "Time", y ="Nitrogen (ug)", title = "Nitrogen [] over Time", color = "Lake")

```

```{r Visualize Data Plots P, echo = FALSE, fig.align = "Left", fig.cap = "P (ug) over Time"}

ggplot(data = data_clean, aes(x = sampledate, y = tp_ug, colour = lakename)) + geom_line(linetype = 5, size  = 0.8) +
  labs(x = "Time", y ="Phosporous (ug)", title = "Phosporous [] over Time", color = "Lake")
...
> The data shows that both Nitrogen and Phosphorous have similar trends in concentrations over time. Nitrogen is found in signicantly more abundance than Phosphorous according the tables above. The mean concentration for Nitrogen is 610.9 ug while that of Phosphorous is 18.9 ug. Seasonally, both Nitrogen and Phosphorous drastically increase in middle of the year (Summer/Fall) for all lakes. However, for the year 1996, both Nitrogen and Phosphorous concentrations peaked for all lakes except Paul Lake and Tuesday Lake (concentrations of 2870.3 ug and 101.1 ug, respectively). 

---

> Moving forward, I would conduct a time-series analysis to determine if there is an overall increase in Nitrogen and Phosphorous across all lakes and for each lake in this study. Since Paul Lake and Tuesday Lake do not share the same pattern as other lakes, a generalized linear model would also be another option for future analysis. If the lake name has a significant effect in determining Nitrogen and Phosphorous, then it's possible that the physical, geographical, or social uses of the lake impact their Nitrogen and Phosphorous concentrations.  
