...
PeterPaul.chem.nutrients <- 
  read.csv("./Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv", stringsAsFactors = TRUE)
EPAair <- read.csv("./Processed/EPAair_O3_PM25_NC1819_Processed.csv", stringsAsFactors = TRUE)
...
ggplot(data = filter(PeterPaul.chem.nutrients, depth == 0), aes(x = daynum, y = temperature_C)) + geom_point()

# Color your points by year, and facet by lake in two rows.
ggplot(data = filter(PeterPaul.chem.nutrients, depth == 0), aes(x = daynum, y = temperature_C, colour = year4) ) + geom_point() + facet_wrap(vars(lakename))

# Change the ylab name
ggplot(data = filter(PeterPaul.chem.nutrients, depth == 0), aes(x = daynum, y = temperature_C, colour = year4) ) + geom_point() + facet_wrap(vars(lakename)) + ylab("Temperature(C)")

#2. 
# Plot temperature by date. Color your points by depth.
ggplot(data = PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, colour = depth) ) + geom_point()

# Change the size of your point to 0.5
ggplot(data = PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, colour = depth) ) + geom_point(size = 0.5)
