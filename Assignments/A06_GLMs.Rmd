...
library(ggplot2)
setwd("./Data/Raw")
ntl <- read.csv('NTL-LTER_Lake_ChemistryPhysics_Raw.csv', header = TRUE)
...
> Answer:
H0: Lake depth level has no significant effect in determining mean of recorded lake 
temperatures for the month of July.
Ha: Lake depth level has a significant effect in determining mean of recorded lake 
temperatures for the month of July.
...
is consistent low mean temperatures.
...
glm0 <- glm(data = ntl_july, temperature_C ~ depth)
summary(glm0)
...
significantly low (less than 0.05). The linear model produce explains around 
73.87% of the variability in mean recorded temperature.
...
glm1 <- glm(data = ntl_july, temperature_C ~ year4 + daynum + depth)

step(glm1, direction = 'backward')

#10

glm_final <- step(step(glm1, direction = 'backward'))
summary(glm_final)
